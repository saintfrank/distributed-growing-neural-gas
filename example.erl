...
%% Exported Functions
%%
-export([start/0, node_life/1, man_life/1 ]).
...
node_life(POSITION)->
	...
	receive 	
		...
		{position_request, MAN_ADDR} -> 
			io:format(" I'm sending my position: ~p to ~p ~n", [POSITION,MAN_ADDR]),
			MAN_ADDR ! {my_pos, self(), POSITION }    %%  this corresponds to a method > position_t givemeposition()
