...
start(FILE_NAME) ->
    ...
    start(FILE, 100, 88, 600, 5.0e-2,
	  5.99999999999999947438e-4, 5.0e-1, 9.995e-1, 1.0e-1,
	  1.0e-1,
	  2.99999999999999988898e-1).%TODO set those

start(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N,
      ALPHA, D, FINAL_ERROR, FINAL_DELTA_ERROR,
      FINAL_DELTA_ERROR_DECAY) ->
    start(FILE, 100, 88, 600, 5.0e-2,
	  5.99999999999999947438e-4, 5.0e-1, 9.995e-1, 3, 5, 0, 0,
	  FINAL_ERROR, FINAL_DELTA_ERROR, FINAL_DELTA_ERROR_DECAY,
	  err_man, win_man, [], 0, 0).

start(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N,
      ALPHA, D, M, N, 0, 0, FINAL_ERROR, FINAL_DELTA_ERROR,
      FINAL_DELTA_ERROR_DECAY, _ERR_MAN, _WIN_MAN,
    ...
    start(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N,
	  ALPHA, D, M, N, FINAL_ERROR, FINAL_DELTA_ERROR,
	  FINAL_DELTA_ERROR_DECAY, 0, 2, ERR_MAN, WIN_MAN,
	  LOST_NODE_LIST, LAST_ERROR, DELTA_ERROR);
%% 	countIteration := 0
%% 	countNodes := 0
...
start(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N,
    ...
    if REM_IT == 0 ->
	   ...
	   checkMessages(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K,
			 MOVE_N, ALPHA, D, M, N, FINAL_ERROR, FINAL_DELTA_ERROR,
			 FINAL_DELTA_ERROR_DECAY, ITERATION + 1,
...
checkMessages(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K,
	      MOVE_N, ALPHA, D, M, N, FINAL_ERROR, FINAL_DELTA_ERROR,
	      FINAL_DELTA_ERROR_DECAY, ITERATION, NUMBER_OF_NODES,
    ...
    receive
	  ...
	  checkMessages(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K,
			MOVE_N, ALPHA, D, M, N, FINAL_ERROR, FINAL_DELTA_ERROR,
			FINAL_DELTA_ERROR_DECAY, ITERATION, NUMBER_OF_NODES - 1,
			ERR_MAN, WIN_MAN,
			lists:append([LOST_NODE_LIST, MANAGER_PID]), LAST_ERROR,
			DELTA_ERROR)
      after 1 ->
		start(FILE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N,
		      ALPHA, D, M, N, FINAL_ERROR, FINAL_DELTA_ERROR,
...
get_coordinates([HEAD | TAIL], TEMP) ->
    if HEAD == 9; HEAD == 10 ->
       ...
       true ->
	   io:format("New number added : ~w\t~w~n",
		     [TEMP, [list_to_integer([HEAD])]]),
	   get_coordinates(TAIL,
			   TEMP ++
			     [list_to_integer([HEAD])])                        %%io:format("New number added : ~w~n",[TEMP])
    end.
