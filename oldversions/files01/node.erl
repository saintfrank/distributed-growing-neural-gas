...
%% 	io:format(" I'm back to life ~p to ~p ~n", [POSITION,self()]),
	...
	receive 	
		...
		{request_decreased_error_and_position, REQUESTING_PID, ALPHA}->
			...
			REQUESTING_PID ! {my_decreased_error, self(), {ERROR*ALPHA, POSITION}},
		...
		{ageEdges, MAX_AGE} ->
			...
			if
				TMP == [] -> 
					...
					io:format(" I'm dead, because i have no friends :( ~n");
				true ->
...
ageEdges([{NODE, AGE}|TAIL], MAX_AGE) ->
	if
		AGE + 1 > MAX_AGE ->
			NODE ! {removeEdge, self(), self()},%TODO do i have to wait for response?
...
moveNeighbors(ok, _, _)-> io:format(" all neighbors are moved ");
