...
err_man_life( PARENT, CHILDREN, NODES, DEPTH,  TWIN)->
	...
	receive 
		...
		{ removeNode, NodePID } -> 
			io:format("~p ERR:I received Remove Node :~p~n",[self(),NodePID]),
		...
		{ addNode, NEW_NODE } -> %% this is received just form the tree
			...
			io:format("~p Received add_node , i've told my twin ~p , my nodes :~w~n",[self(), TWIN,NODES]),
			err_man_life( PARENT, CHILDREN, NEW_NODES_LIST, DEPTH,  TWIN);  %%TODO i've to add this in the win_man
 		
		{decrease_error, PARENT ,ALPHA} ->
			io:format("~p Received decrease_error, i'll tell everyone:~n",[self()]),
			tell_everyone(CHILDREN, { decreaseError, self(), ALPHA } ),
			tell_everyone(NODES, { request_error, self(),ALPHA }),
			err_man_life( PARENT, CHILDREN, NODES, DEPTH,  TWIN);

		{request_decreased_error, PARENT,ALPHA} ->	

			io:format("~p Received request_decreased_error, i begin calculate the max:~n",[self() ]),
			...
			io:format("~p I asked about the error. My Nodes: ~w My CHildren: ~w:~n",[self(), NODES, CHILDREN ]),
		
        		%% NUM_ANSWERS_ARRIVING is a number that tell how many distances to wait
        		NUM_ANSWERS_ARRIVING= length(NODES) + length(CHILDREN),
			io:format("~p Waits : ~p~n",[self(), NUM_ANSWERS_ARRIVING]),
			...
			io:format("~p My error: ~w~n",[self(), MAX]),
		...
		{ move_nodes_start, NEW_NODE, DIRECTIONS} ->
			...
			io:format("~p Received move_node_start, i'll pass :~w~n",[self(), NODES_PACKAGE]),
			NEW_NODES_LIST = [],
		...
		{move_nodes, NODES_PACK, DIRECTIONS}->   %% By-passing manager 
			io:format("~p Received move_node, i'll pass :~w~n",[self(), NODES_PACK]),
...
add_manager(DEPTH, TWIN) ->
	...
	TWIN ! { update_add_manager, NEW_MAN } ,
	
	receive
		{ update_add_manager_confirm , WIN_TWIN } ->
			NEW_MAN ! {set_twin, WIN_TWIN},
			NEW_MAN
		after 4000 -> io:format("~w :I've added a node but I didn't receive confirmation from my twin ~w~n",self(),TWIN)
	end
.
