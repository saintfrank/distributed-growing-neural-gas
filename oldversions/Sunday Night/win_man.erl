...
win_man_life( PARENT, CHILDREN, NODES)->
	...
	receive	
		...
		{ removeNode, NodePID } -> 
			io:format("~p WIN: I received Remove Node :~p~n",[self(),NodePID]),
		...
		{new_input, PARENT, INPUT } ->
			...
			io:format("~p Received New Input ~w, nodes:~w children:~w:~n",[self(), INPUT, NODES, CHILDREN]),
			...
			io:format("~p Win coming form the nodes ~w:~n",[self(),WIN_AND_RUN ]),
	

			COMPARED_WIN_AND_RUN = compare_winners(length(CHILDREN), element(1,WIN_AND_RUN), element(2,WIN_AND_RUN)),

			io:format("~p Win coming form the children ~w:~n",[self(),WIN_AND_RUN ]),
...
compute_winner(NODES_REMAINING, TEMP_WIN, TEMP_RUN)->
	io:format("I'm waiting ~p~n",[NODES_REMAINING]),
	receive 
		{my_distance, NODE_PID, DISTANCE} ->
		 	io:format("my_distance arrived ~n"),			
		...
		after 100 -> io:format("Still waiting ...")
	end
.
