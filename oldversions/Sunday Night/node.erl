...
%% 	io:format(" I'm back to life ~p to ~p ~n", [POSITION,self()]),
	...
	receive 	
		...
		{request_position, REQUESTING_PID} -> 
			io:format(" I'm sending my position: ~p to ~p ~n", [POSITION,REQUESTING_PID]),
			REQUESTING_PID ! {my_position, self(), POSITION },
			node_life(POSITION, EDGES, ERROR, PARENT, ENGINE);
		
		{request_edges, REQUESTING_PID} -> %TODO has to be REQUESTING_PID_2 ??? 
			io:format(" I'm sending my edges: ~p to ~p ~n", [EDGES,REQUESTING_PID]),
			REQUESTING_PID ! {my_edges, self(), EDGES },
			node_life(POSITION, EDGES, ERROR, PARENT, ENGINE);
		
		{request_error, REQUESTING_PID} -> 
			io:format(" I'm sending my error: ~p to ~p ~n", [ERROR,REQUESTING_PID]),
		...
		{request_decreased_error_and_position, REQUESTING_PID, ALPHA}->
			...
			io:format(" I returned my error and position, i am ~p ~n",[self()]),
			node_life(POSITION, EDGES, ERROR*ALPHA, PARENT, ENGINE);
		
		{newEdge, OTHER_NODEPID} ->	%has to be added also in the otherNode
			io:format(" I have created a new Edge with ~p ~n", [OTHER_NODEPID]),
		...
		{ageEdges, MAX_AGE} ->
			...
			if
				TMP == [] -> 
					...
					io:format(" ~p I'm dead, because i have no friends :( ~n",[self()]),
					exit("no friends");
				true ->
		...
		{addToError, DELTA_ERROR} ->
			io:format(" my error is now ~p bigger ~n", [DELTA_ERROR]),
			node_life(POSITION, EDGES, ERROR + DELTA_ERROR, PARENT, ENGINE);
		
		
		{decreaseErrors, ALPHA} ->
			io:format(" I decreased my error to a ~p part ~n", [ALPHA]),
			node_life(POSITION, EDGES, ERROR*ALPHA, PARENT, ENGINE);

		
		{moveKN, INPUT, MOVE_K, MOVE_N} ->
			moveNeighbors(EDGES, INPUT, MOVE_N),
			io:format("I moved ~p and hopefully my neighbors ~n", [MOVE_K]),
...
posToStr([], FILE)->
	io:format(FILE, "~n"),
	file:close(FILE);
posToStr([HEAD|POSITION], FILE)->
		io:format(FILE, "~f\t",[HEAD]),
		posToStr(POSITION, FILE)
.	
...
ageEdges([{NODE, AGE}|TAIL], MAX_AGE) ->
	if
		AGE + 1 > MAX_AGE ->
			NODE ! {removeEdge, self()},%TODO do i have to wait for response?
...
ageAnEdge(ok, OTHER_NODE)->
	io:format("NODE_ageAnEdge there is no edge to ~p~n",[OTHER_NODE]);
...
removeEdge(OTHER_NODE, [{OTHER_NODE, _}|TAIL]) ->
	removeEdge(OTHER_NODE, TAIL),  %TO DO should this send confirmation?
...
moveNeighbors(ok, _, _) -> io:format("NODE:moveNeighbors ok is strange~n");
moveNeighbors([], _, _)-> io:format(" all neighbors are moved ~n");
...
refreshEdge(OTHER_NODE, ok) ->
	io:format("edge to ~p was created instead of refreshed ~n", [OTHER_NODE]),
