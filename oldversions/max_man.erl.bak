...
max_man_life(PARENT, CHILDERN, NODES, NUM_N, NUM_C,
	     DEPTH, INPUT) ->
...
compute_max ( 0 , TEMP_MAX , TEMP_SECOND ) -> { TEMP_MAX , TEMP_SECOND } ; compute_max ( NODES_REMAINING , TEMP_MAX , TEMP_SECOND ) -> receive { my_distance , NODE_PID , DISTANCE } -> DISTANCE_TEMP_MAX = element ( 2 , TEMP_MAX ) , if ( DISTANCE_TEMP_MAX > DISTANCE ) -> TEMP_SECOND = TEMP_MAX , NEW_MAX = { NODE_PID , DISTANCE } , compute_max ( NODES_REMAINING - 1 , NEW_MAX , TEMP_SECOND ) ; true -> DISTANCE_TEMP_SECOND = element ( 2 , TEMP_SECOND ) , if ( DISTANCE_TEMP_SECOND > DISTANCE ) -> NEW_SECOND = { NODE_PID , DISTANCE } , compute_max ( NODES_REMAINING - 1 , TEMP_MAX , NEW_SECOND end end end .
