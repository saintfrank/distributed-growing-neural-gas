...
start(NAME)->
	...
	spawn(writing,write_position,[11.213213413,File]),
	spawn(writing,write_position,[21.213423541,File]),
	spawn(writing,write_position,[31.213412341,File]),
...
posToStr([], FILE)->
	io:format("NODE  nothing to print anymore, i return ~n"),
	io:format(FILE, "~n",[]),
	[];
posToStr([HEAD|TALE], FILE)->
		io:format(FILE, "~p\t",[HEAD]),
		io:format("NODE  i print: ~p\t , tale is ~p ~n", [HEAD,TALE]),
		posToStr(TALE, FILE)
.
...
posToStr2([HEAD|TALE])->
		...
		string:sub_string(float_to_list(HEAD),0,8) ++ "      " ++ posToStr2(TALE)
.
