...
start([FILE_NAME])->
	...
	case file:open(FILE_NAME, [read]) of
	    {ok, FILE}	 ->
			  case file:open(nodes.dat, [write]) of
				    {ok, FILE_WRITE}	->
							io:format(" file ~p opened as ~p~n",[FILE_NAME, FILE]),
							%	 88	 600  0.05						0.1
			  				start(FILE, FILE_WRITE, 100, 10, 3, 0.5, 0.0006, 0.5, 0.9995, 1, 5, 0.0001);
				    {error, WHY}	-> 
							io:format("Problems opening the file nodes.dat because ~p~n",[WHY])
			  end; 
	    {error, WHY} ->
			  io:format("Problems opening the file ~p because ~p~n",[FILE_NAME, WHY])
	end
	.%TODO set those
...
start(FILE, FILE_WRITE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N, ALPHA, D, FINAL_ERROR, FINAL_DELTA_ERROR,
	...
	start(FILE, FILE_WRITE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N, ALPHA, D, 3, 5, 0, 0, FINAL_ERROR, FINAL_DELTA_ERROR,
...
start(FILE, FILE_WRITE, MAX_NODES, MAX_AGE, LAMBDA, MOVE_K, MOVE_N, ALPHA, D, M, N, ITERATION, NUMBER_OF_NODES,
	...
	io:format("****************************************~n~nLOOP we're in the loop I = ~p and have ~p Nodes~n~n****************************************~n",[ITERATION, NUMBER_OF_NODES]),
	...
	%% Reset the file 
	if 
	     ITERATION rem 600  == 50 ->
		      file:copy("nodes.dat","nodes2.dat");
	     ITERATION rem 600  == 100 ->
...
%% 
	...
	io:format("ENGINE / LOOP waiting for the \"addEdges_done\" message~n"),
		
	receive
		{ageEdges_done, WINNER} -> ok
	end,
	
	io:format("ENGINE / LOOP : message receive. checking if we're in the lambda'th iteration~n"),
	
	REM_IT = (trunc(ITERATION) rem trunc(LAMBDA)),
	io:format("ENGINE / LOOP REM_IT calculated : ~p~n",[REM_IT]),
	if
		REM_IT == 0 ->
			...
			if
				ERROR < FINAL_ERROR ->
					io:format("LOOP with ~p nodes a maximal error of ~p is reached~n",[NUMBER_OF_NODES, ERROR]),
				...
				(DELTA_ERROR < FINAL_DELTA_ERROR) ->%DELTA_ERROR = DELTA_ERROR*ERROR_DECAY + abs(LAST_ERROR - ERROR)
					io:format(
					"LOOP maximal number (~p) of nodes reached, and the maximal error stays the same at about ~p~n",
				...
				true ->
					...
					F ! {request_decreased_error_and_position, self(), ALPHA},
...
addNode(ERR_MAN, NEW_NODE, NUMBER_OF_NODES, M, N, [])->
	...
	if
		...
		true ->
			io:format("addNODE ENGINE_ADDNODE sends addNode~p~n",[NEW_NODE]),
			ERR_MAN ! {addNode, NEW_NODE}
	end;
addNode(_ERR_MAN, NEW_NODE, _NUMBER_OF_NODES, _M, _N, [HEAD|_LOST_NODE_LIST])->
	io:format("addNODE ENGINE_ADDNODE sends addNode, to first element of LOST_NODE_LIST~n"),
