...
win_man_life( PARENT, CHILDREN, NODES)->
	...
	receive	
		...
		{ removeNode, NodePID } -> 
			io:format("W_M ~p WIN: I received Remove Node :~p~n",[self(),NodePID]),
			NodePID ! i_removed_you,			
			win_man_life( PARENT, CHILDREN, lists:delete(NodePID, NODES));
		...
		{ update_add_manager, NEW_ERR_MAN , MYTWIN} ->
			...
			io:format("W_M ~p I add a manager ! ~w ~n:",[self(), CHILDREN]),
			
			NEW_MAN = spawn( win_man, win_man_life, [self(),  % new parent
						[],	% new NODES list
						[],	% new CHILDREN list	
						NEW_ERR_MAN]),	%%  he already know who i sthe twin 

			NEW_CHILDREN= lists:append(CHILDREN, [NEW_MAN]),

			io:format("W_M ~p manager added ! ~w ~n:",[self(), NEW_CHILDREN]),
		...
		{ update_add_nodes_pack, NODES_PACK } ->
			io:format("W_M ~p I add a nodes pack ! ~w ~n:",[self(), NODES_PACK]),
			win_man_life(PARENT, CHILDREN, NODES_PACK );
		
		%% 		
		{new_input, PARENT, INPUT } ->

			io:format("W_M ~p Received New Input ~w, nodes:~w children:~w:~n",[self(), INPUT, NODES, CHILDREN]),
			...
			io:format("W_M ~p Win coming form the nodes ~w:~n",[self(),WIN_AND_RUN ]),
	

			COMPARED_WIN_AND_RUN = compare_winners(length(CHILDREN), element(1,WIN_AND_RUN), element(2,WIN_AND_RUN)),

			io:format("W_M ~p Win coming form the children ~w:~n",[self(),WIN_AND_RUN ]),
			...
			io:format("W_M ~p Return  to my parent ~w the winner and runner,~w my nodes:  ~w:~n",[self(), PARENT,COMPARED_WIN_AND_RUN,NODES]),
...
compute_winner(NUM_N) -> 
		compute_winner(NUM_N,{0,1000000000000},{0,100000000000000}).  %% TODO find a better solution
			...
			if   
				...
				true ->  
					...
					if
						...
						true-> 	compute_winner(NODES_REMAINING - 1, TEMP_WIN, TEMP_RUN)			        
					end
			end
